...
function setCenterIcon(points, myMap, offsetX, offsetY) {
    for (let i of points) {
        var pixelCenter = myMap.getGlobalPixelCenter(i.coords.split(","));

        pixelCenter = [
            pixelCenter[0] + offsetY,
            pixelCenter[1] + offsetX
        ];
    ...
    document.addEventListener("ya-maps:load", () => {
      ...
      for (let i of points) {
        ...
        if (document.documentElement.clientWidth > 992) {
            setCenterIcon(points, myMap, 0, -300);
        } else if (document.documentElement.clientWidth > 768) {
            setCenterIcon(points, myMap, 0, -150);
        }
